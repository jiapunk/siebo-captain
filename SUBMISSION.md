...
- 合作网络：9 节点 / 6 边 / 聚类系数 0.48（标准平均聚类：degree <2 的节点记 0 并计入平均；旧版 0.87 是排除孤立节点的算法，已作废）；社交 vs 能力双信号模拟（10 个假设；旧数据里同一组队友的反向重复记录只算一次）：social 聚类 0.55（+2 边）、competence 0.48（+0 边），两种信号选出不同队伍
